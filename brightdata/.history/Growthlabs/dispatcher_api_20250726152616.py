...
class BrightDataAPI:
    ...
    def __init__(self):
        ...
        self.results_url = "https://api.brightdata.com/datasets/v3/trigger/result"
        self.headers = {
    ...
    def search_and_scrape(self, keyword: str, num_posts: int = 100) -> List[Dict[str, Any]]:
        ...
        try:
            print(f"Making Bright Data API request for keyword: {keyword}")
            ...
            # Wait a bit for the snapshot to be processed
            import time
            time.sleep(5)
            
            # Now fetch the results using the snapshot ID
            return self._fetch_snapshot_results(snapshot_id)
    ...
    def _fetch_snapshot_results(self, snapshot_id: str) -> List[Dict[str, Any]]:
        """Fetch results from a snapshot ID."""
        try:
            params = {
            ...
            print(f"Fetching results for snapshot_id: {snapshot_id}")
            ...
            for attempt in range(10):  # Increased to 10 attempts
                ...
                print(f"Fetch attempt {attempt + 1}, status: {response.status_code}")
                
                if response.status_code == 200:
                    result = response.json()
                    print(f"Fetch response keys: {list(result.keys())}")
                    ...
                    print(f"Snapshot {snapshot_id} not ready yet, waiting...")
                    # Exponential backoff: 15, 30, 60, 120, 240 seconds
                    wait_time = min(15 * (2 ** attempt), 300)  # Max 5 minutes
            ...
            print(f"Failed to fetch results after 10 attempts")
            return []
            
        except Exception as e:
            print(f"Error fetching snapshot results: {e}")
            return []
