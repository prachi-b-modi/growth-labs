...
DEFAULT_DISPATCHER_SECRET = "your_shared_secret"
...
def push_targets_to_dispatcher(distinct_ids: List[str],
                               window_label: str,
                               dispatcher_url: str,
                               dispatcher_secret: str,
                               mode: str = "replace") -> None:
    """
    POST the selected IDs to your FastAPI Dispatcher at /targets/bulk_sync.
    """
    url = f"{dispatcher_url.rstrip('/')}/targets/bulk_sync"
    payload = {"window": window_label, "distinct_ids": distinct_ids, "mode": mode}
    r = requests.post(url, params={"secret": dispatcher_secret}, json=payload, timeout=60)
    if r.status_code != 200:
        raise SystemExit(f"Dispatcher error {r.status_code}: {r.text[:400]}")
    print("Bulk sync:", r.status_code, r.text)
